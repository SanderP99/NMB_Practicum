f1 = @(x) cos(x);
f2 = @(x) 1./(25*x.^2+1);
figure('Renderer', 'painters', 'Position', [10 10 1500 600], 'PaperPositionMode', 'auto');
set(gca, 'Units', 'normalized','FontUnits','points','FontWeight','normal','FontSize',14,'FontName','Times')
hold on
fplot(f1)

% Punten om te evalueren
x = zeros(1,200);
for i = 1:1:200
    x(i) = -1 + 1/200 + 2/200*(i-1);
end

for n = [3,4,6,8,10,60]
    N = n;
    % Equidistante punten
    x_equi = zeros(1,N);
    for i = 1:1:N
        x_equi(i) = -1 + (i-1) .* 2/(N-1);
    end
    x_equi(N) = 1;

    % Nulpunten
    x_nul = zeros(1,N);
    for i = 1:1:N
        x_nul(i) = cos(pi*(2*i-1)/(2*N));
    end

    [c, kappa] = interpolate(x_nul, f1);
    interpolant = evalCheb(c, x);

    plot(x, interpolant)
end
legend({'$f(x) = \cos(x)$', '3', '4', '6', '8', '10', '60'}, 'interpreter', 'latex', 'location', 'south', 'NumColumns', 3, 'FontSize', 20,'FontWeight', 'normal', 'FontName', 'Times');
hold off
axis([-1 1 0 1]);

print  ('-r500', '-depsc2', 'cos_nul_laag.eps')